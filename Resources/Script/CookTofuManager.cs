...
public class CookTofuManager : MonoBehaviour
{
    enum CookingTofu
    {
        Start = 0,
    ...
    [SerializeField] CookingTofu ing;
    ...
    [SerializeField] GameObject[] tofus;
    ...
    [SerializeField] GameObject eggtofupos;
    //[SerializeField] GameObject oilinpos;
    [SerializeField] GameObject tofuinpos;
        ...
        switch (ing)
        {
            case CookingTofu.Start:
                ...
                else
                {
                    ...
                    {
                        ing = CookingTofu.CutTofu;
                    }
                }
                break;
            case CookingTofu.CutTofu:
                if(flag)
                {
                ...
                else
                {
                    if(stay)
                    {
                        if(tempcount<cutlines.Length)
                        {
                            ...
                            {
                                tofus[tempcount].GetComponent<Rigidbody>().isKinematic = false;
                                tofus[tempcount].GetComponent<BoxCollider>().enabled = true;
            ...
            case CookingTofu.EggInfo:
                ...
                else
                {
                    ...
                    if (RHand.grabPinchAction.GetStateDown(SteamVR_Input_Sources.RightHand) || LHand.grabPinchAction.GetStateDown(SteamVR_Input_Sources.LeftHand))
                    {
                        ing = CookingTofu.BrokenEgg;
                    }
                }
                break;
            case CookingTofu.BrokenEgg:
                if (flag)
                {
                    uimanager.ShowNextUI();
            ...
            case CookingTofu.MixEgg:
                ...
                break;
            case CookingTofu.EggInTofu:
                if (flag)
                {
                    uimanager.ShowNowUI();
                    eggtofupos.SetActive(true);
                ...
                {
                    if (stay && eggtofupos.GetComponent<GoalInfoObject>().GetCount() >= 6)
                    {
                        eggtofupos.SetActive(false);
                        uimanager.ShowResultUI(0);  //잘했습니다
                        Invoke("StayTime", staytime);
                        stay = false;
                    }
            ...
            case CookingTofu.PlusTemp:
                ...
                break;
            case CookingTofu.MoveTofu:
                if (!flag)
                {
                    uimanager.ShowNowUI();
                    tofuinpos.SetActive(true);
                    stay = true;
                    flag = true;
                }
                else
                {
                    if (stay && tofuinpos.GetComponent<GoalInfoObject>().GetCount() >= 6)
                    {
                        tofuinpos.SetActive(false);
                        uimanager.ShowResultUI(0);  //잘했습니다
                        Invoke("StayTime", staytime);
                        stay = false;
                    }
            ...
            case CookingTofu.Stay:
                ...
                else
                {
                    ...
                    if (stay)
                    {
                        ...
                        {
                            for (int i = 0; i < tofus.Length; i++)
                            {
                                tofus[i].GetComponent<Animator>().SetBool("T_C", true);
                            }
            ...
            case CookingTofu.MoveFried:
                ...
                else
                {
                    ...
                    {
                        if (friedpos.GetComponent<GoalInfoObject>().GetCount() >= 6)
                        {
    ...
    void StartSetting()
    {
        ...
        for (int j = 0; j < tofus.Length; j++)
        {
            tofus[j].GetComponent<Rigidbody>().isKinematic = true;
            tofus[j].GetComponent<BoxCollider>().enabled = false;
        }
